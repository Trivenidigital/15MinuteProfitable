...
for i in 1 2 3 4 5 6 7; do
  ...
  echo ""
  echo "--- Service ---"
  systemctl is-active btc15minutebot

  echo ""
  echo "--- Memory ---"
  systemctl status btc15minutebot --no-pager 2>&1 | grep Memory

  echo ""
  echo "--- Trades Since Restart ---"
  sqlite3 -header -column /opt/btc15minutebot/data/trades.db \
  ...
  SL=$(tail -50000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c 'stop_loss_triggered' || echo 0)
  TP=$(tail -50000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c 'take_profit_triggered' || echo 0)
  TE=$(tail -50000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c '"event": "time_exit"' || echo 0)
  LT=$(tail -50000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c 'time_exit_skipped_lottery' || echo 0)
  ...
  echo ""
  echo "--- Errors ---"
  ERR=$(tail -5000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c '"level": "error"' || echo 0)
  CB=$(tail -5000 /var/log/btc15minutebot/bot.log 2>/dev/null | grep -c 'circuit_breaker' || echo 0)
